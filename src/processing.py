...
def filter_by_state(dictionary_list: list, state: str = 'EXECUTED') -> list:
    ...
    Возвращает новый список, содержащий только те словари, у которых значение ключа 'state' соответствует переданному.
...
def sort_by_date(dictionary_list: list, sort_method: str = 'latest'):
    """
        ...
        :param dictionary_list: Список словарей, каждый из которых содержит ключ 'date'.
        :param sort_method: Метод сортировки. 'latest' (по умолчанию) — от новых к старым, 'earliest' — от старых к новым.
