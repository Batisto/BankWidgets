...
def test_transactions_descriptions_valid(transactions):
    expected = ["Перевод организации", "Перевод организации", "Перевод организации"]
...
def test_card_number_generator_formatting():
    ...
    result = list(card_number_generator(start, end))
    assert all(
        len(card.split()) == 4 for card in result
    )  # Проверяем, что каждая строка состоит из 4 частей
    assert all(
        all(len(part) == 4 for part in card.split()) for card in result
    )  # Каждая часть должна быть длиной 4 символа
...
def test_card_number_generator_edge_case_max():
    ...
    expected = ["9999 9999 9999 9999", "1000 0000 0000 0000"]
    result = list(card_number_generator(start, end))
    assert result == expected
